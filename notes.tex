...
\end{equation}  
where for $N=0$, we interpret $\h_1\otimes\cdots\otimes\h_N$ just as $\C$, being the 0-particle space. The vector $1 \in \C$ is called the vacuum vector and is denoted by $\Omega$ or $\bra{\Omega}$. The Hamilton operator on the Fock space can be written as $H=\oplus_{N=0}^{\infty}H_N$
