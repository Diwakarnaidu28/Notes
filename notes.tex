...
\end{equation}
Then the two projections $P_{\pm}$ define subspaces of $\NN_N$, i.e., the symmetric and antisymmetric tensor products\footnote{For the ease of notation, we take the particles to be identical.}
\begin{equation}
	\bigotimes\limits_{\mathrm{symm}}^N\h \coloneq P_+\left(\bigotimes^{N}\h\right) , \quad 
	\bigwedge\limits^N\h \coloneq P_-\left(\bigotimes^{N}\h\right) \,.
\end{equation}
