...
\end{equation}
On $\HH_N$, we have a natural action of the symmetric group $S_N$. That is for a $\sigma \in S_N$, we have a unitary map $U_\sigma: \HH_N \rightarrow\HH_N $ uniquely defined by its action on the tensor product. So for $\phi = \phi_1\otimes\ldots\otimes\phi_N \in \HH_N$, the $N$-particle wavefunction which is the tensor product of one particle wavefunctions, we have
\begin{equation}
...
\end{equation}
Then the two projections $P_{\pm}$ define subspaces of $\NN_N$, i.e., the symmetric and antisymmetric tensor products
\begin{equation}
	\bigotimes\limits_{\mathrm{symm}}^N\h \coloneq P_+(\h_1\otimes\cdots\otimes\h_N) , \quad 
	\bigwedge\limits^N\h \coloneq P_-(\h_1\otimes\cdots\otimes\h_N) \,.
\end{equation}
...
Observe that in the definition of the $N$ particle Hilbert space, the particle number stays fixed. If we have an infinte sequence of particles, we can also consider all particle numbers at once. To this end, we introduce the Fock Hilbert space  
Here we choose the one particle Hilbert spaces to be $L^2(\R^3)$, which gives us
\begin{equation}
